...
if [ -n "$HAPPYCATTLE_TOKEN_KEY_CONTENT" ]; then
    ...
    key=`(echo ${HAPPYCATTLE_TOKEN_KEY_CONTENT} | awk '/-----BEGIN PRIVATE KEY-----/, /-----END PRIVATE KEY-----/')`
elif [ -n "$HAPPYCATTLE_TOKEN_KEY_FILE" ]; then
    # Private Key set as file
    echo "Private key set as file, replacing in api-umbrella.yml"
    key=`(awk '/-----BEGIN PRIVATE KEY-----/, /-----END PRIVATE KEY-----/' /run/secrets/happycattle_key)`
else
    # No private Key set
...
sed -i "s:<HAPPYCATTLE_KEY>:${key3}:g" api-umbrella.yml
...
if [ -n "$HAPPYCATTLE_TOKEN_CRT_CONTENT" ]; then
    ...
    # Replace line breaks
    crt_oneline=`(echo ${HAPPYCATTLE_TOKEN_CRT_CONTENT} | awk 'NF {sub(/\r/, ""); printf "%s",$0;}')`
elif [ -n "$HAPPYCATTLE_TOKEN_CRT_FILE" ]; then
    # Certificate set as file
    echo "Certificate set as file, replacing in api-umbrella.yml"
    # Replace line breaks
    crt_oneline=`(awk 'NF {sub(/\r/, ""); printf "%s",$0;}' /run/secrets/happycattle_cert)`
else
    # No certificate set
...
# Replace var in yml template
sed -i "s:<HAPPYCATTLE_CERTS>:${crt_finalarray}:g" api-umbrella.yml


# Set EORI ENV
if [ -n "$HAPPYCATTLE_EORI" ]; then
    # Replace var in yml template
    sed -i "s/<HAPPYCATTLE_EORI>/${HAPPYCATTLE_EORI}/g" api-umbrella.yml
else
...
while ! curl --silent --fail happycattle-elasticsearch.docker:9200/_cluster/health;
do
